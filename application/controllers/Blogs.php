...
class Blogs extends CI_Controller{
    ...
    public $_table_column_array = ['title','cover','content','published','status'];
    public $_table_column_edit = ['id','title','cover','content','published','status'];
    public $required = ['id'];
       ...
       $this->Blogs_model->saveUserLogs($saveLogInfo);
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Blogs_model->getById($_POST['id']);
                if($result != null){
    ...
    public function getTops(){
       ...
       $this->Blogs_model->saveUserLogs($saveLogInfo);
        $auth  = $this->input->get_request_header('Basic');
        if($auth && $auth == $this->config->item('encryption_key')){
            $result = $this->Blogs_model->getTops();
            if($result != null){
                echo $this->json->response($result,$this->_OKmessage,$this->_statusOK);
            }else{
                echo $this->json->response($this->db->error(),$this->_Errmessage,$this->_statusErr);
    ...
    public function editList(){
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Blogs_model->editList($_POST,$_POST['id']);
    ...
    public function save(){
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Blogs_model->saveList($_POST);
                if($result != null){
                    $id = $this->db->insert_id();
                    $data = $this->Blogs_model->getByIdValue($id);
                    echo $this->json->response($data,$this->_OKmessage,$this->_statusOK);
                }else{
                    echo $this->json->response(['error'=>'Something Went Wrong.'],$this->_Errmessage,$this->_statusErr);
                }
    ...
    public function deleteList(){
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Blogs_model->deleteList($_POST['id']);
                if($result != null){
                    echo $this->json->response($result,$this->_OKmessage,$this->_statusOK);
                }else{
                    echo $this->json->response(['error'=>'Something Went Wrong.'],$this->_Errmessage,$this->_statusErr);
